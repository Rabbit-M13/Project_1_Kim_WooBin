...
public:
	...
	/// </summary>
	inline void SetMaxMagNumber() { MaxMagNumber += CurrentRound; };
	...
	/// 탄창을 비운다.
	/// </summary>
	inline void ClearMagazine() { Magazine.clear(); }
...
private:
	...
	int BlankBullet = 500; // (공포탄 수 int) 라운드 별 숫자를 고정으로할지 고민 중
	int LiveBullet = 500;	// (실탄 수 int) 라운드 별 숫자를 고정으로할지 고민 중
