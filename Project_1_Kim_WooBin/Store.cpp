		...
		case 1: // 화약
			{
				...
				if (InvenIndex["화약"] >= InItems->GetMaxItemCount())
				{
					InvenIndex["화약"] = InItems->GetMaxItemCount();
		...
		case 2: // 맥주
			{
				...
				// Key(아이템 이름)값을 받아와서 Value가 MaxItemCount를 초과하지 않는지 체크해야함
				auto& InvenIndex = InPlayer->GetInventory();
				if (InvenIndex["맥주"] >= InItems->GetMaxItemCount())
				{
					InvenIndex["맥주"] = InItems->GetMaxItemCount();
					printf("최대 한도를 소지 중입니다!\n");
					break;
				}
		...
		case 3: // 수상한 안경
			{
				...
				// Key(아이템 이름)값을 받아와서 Value가 MaxItemCount를 초과하지 않는지 체크해야함
				auto& InvenIndex = InPlayer->GetInventory();
				if (InvenIndex["수상한 안경"] >= InItems->GetMaxItemCount())
				{
					InvenIndex["수상한 안경"] = InItems->GetMaxItemCount();
					printf("최대 한도를 소지 중입니다!\n");
					break;
				}
